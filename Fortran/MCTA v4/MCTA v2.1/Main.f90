program main
    ...
    !以下为输入定日镜结构参数的部分……………………………!   
    write(*,*) "Please type in mirror field type(1 or 2 or 3 or 4 or 5):"
    read(*,*) ftype    
    do while ( ftype /= 1 .and. ftype /= 2 .and. ftype /= 3 .and. ftype /= 4 .and. ftype /= 5)
        write(*,*) "Out of range! Please type again!"
        read(*,*) ftype
        end do 
    ...
    !以下为输入单个定日镜取样光线数目的部分…………………!
    write(*,*) "Please type in sample number (Larger than 100000):"
    read(*,*) sampleN
    do while ( sampleN<100000)
    ...
    if (ftype == 1) then
        ...
        do mirrorN=1,mirrors
            if ((-1)**(mirror(mirrorN)%i) >=0 ) then
                mirror(mirrorN)%x=(((mirror(mirrorN)%j)-1)*MirrorDx+FDmin)*cos((((mirror(mirrorN)%i)-1)*MirrorDy + 0.5*MirrorDy)/(FDmin+(mirror(mirrorN)%j-1)*MirrorDx))
                mirror(mirrorN)%y=(((mirror(mirrorN)%j)-1)*MirrorDx+FDmin)*sin((((mirror(mirrorN)%i)-1)*MirrorDy + 0.5*MirrorDy)/(FDmin+(mirror(mirrorN)%j-1)*MirrorDx))
                mirror(mirrorN)%z=MirrorH
            else
                mirror(mirrorN)%x=(((mirror(mirrorN)%j)-1)*MirrorDx+FDmin)*cos(((mirror(mirrorN)%i)-1)*MirrorDy/(FDmin+(mirror(mirrorN)%j-1)*MirrorDx))
                mirror(mirrorN)%y=(((mirror(mirrorN)%j)-1)*MirrorDx+FDmin)*sin(((mirror(mirrorN)%i)-1)*MirrorDy/(FDmin+(mirror(mirrorN)%j-1)*MirrorDx))
